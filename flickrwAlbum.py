...
class Album(object):
    ...
    def _load_properties(self):
        """Loads the properties from Flickr."""
        method = 'flickr.albums.getInfo'
        data = _doget(method, album_id=self.id)

        self.__loaded = True
        
        album = data.rsp.album

        self.__name = album.name.text
        self.__description = album.description.text
        self.__members = album.members.text
        self.__privacy = album.privacy.text


    def __str__(self):
        return '<Flickr album %s>' % self.id
    
    def getPhotos(self, tags='', per_page='', page=''):
        """Get a list of photo objects for this album"""
        method = 'flickr.albums.pools.getPhotos'
        data = _doget(method, album_id=self.id, tags=tags,\
                      per_page=per_page, page=page)
        photos = []
        for photo in data.rsp.photos.photo:
            photos.append(_parse_photo(photo))
    ...
    def add(self, photo):
        ...
        method = 'flickr.albums.pools.add'
        _dopost(method, auth=True, photo_id=photo.id, album_id=self.id)
        return True

    def remove(self, photo):
        """Remove a Photo from the album"""
        method = 'flickr.album.pools.remove'
        _dopost(method, auth=True, photo_id=photo.id, album_id=self.id)
        return True
